...
module.exports = class WebServer {
    ...
    async oauthJWT(req, res) {
        ...
        if (this.config.isLocalhost) {
            privateKey = fs.readFileSync(path.resolve('../@Private/cert/JWT', 'private.pem')).toString('utf8');
        } else {
    ...
    async getSettings(req, res) {
        let output = {
            ...
            // TOTP: { label: "Time-Base One time Password (TOTP/MFA)", value: process.env.OAUTH_TOTP },
    ...
    _makeServerHTTPS() {
        if (this.config.isLocalhost) {
            const serverHTTPS = require('https').createServer(
                {
                    key: fs.readFileSync(
                        path.resolve('../@Private/cert/WebServer', 'key.pem')
                    ),
                    cert: fs.readFileSync(
                        path.resolve('../@Private/cert/WebServer', 'cert.pem')
                    )
                },
                this.app
            );
